        ...
        **kwargs: Any,
        ...
        try:
            ...
            if not tool_mgr.save_mcp_config(config):
                logger.error(
                    f"enable_mcp_server: save_mcp_config failed for {server_name}"
                )
    ...
    async def call(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            if not tool_mgr.save_mcp_config(config):
                logger.error(
                    f"disable_mcp_server: save_mcp_config failed for {server_name}"
                )
        ...
        **kwargs: Any,
        ...
        try:
            ...
            if not tool_mgr.save_mcp_config(mcp_config):
                logger.error(
                    f"add_mcp_server: save_mcp_config failed for {server_name}"
                )
                return _err("保存配置失败。")
    ...
    async def call(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            if not tool_mgr.save_mcp_config(config):
                logger.error(
                    f"remove_mcp_server: save_mcp_config failed for {server_name}"
                )
                return _err("保存配置失败。")
