...
- AI 工具新增 `skill_scan_mode`；命令先报告、后等待同用户精确回复所选语言的确认词，默认五分钟超时不安装。新增 `--force` 忽略内容分析，但仍须确认并通过包结构检查。
- 所有工具结果包含扫描报告；未指定审查模型的命令在主模型失败时保留报告，后续请求补投，不恢复已过期的安装授权。
- 添加扫描、安装回滚、模型回传和 AstrBot 入口集成测试。

### Changed
- 命令安装不再隐式覆盖同名 Skill；ZIP 更新统一保留原有启用状态。
